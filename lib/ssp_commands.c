...
SSP_RESPONSE_ENUM ssp_aver_que_sirve(SSP_COMMAND_SETUP setup)
{
    SSP_COMMAND sspC;
    SSP_RESPONSE_ENUM resp;
    _ssp_setup_command_structure(&setup, &sspC);
    ...
    {
        sspC.CommandData[0] = i;
        resp = _ssp_return_values(setup.port, &sspC);
        if (resp == SSP_RESPONSE_OK)
            printf("este si jalo %x\n", i);
        sleep(5);
    }
...
SSP_RESPONSE_ENUM ssp_execute_command(SSP_COMMAND_SETUP setup, const unsigned char route)
{
    SSP_COMMAND sspC;
    SSP_RESPONSE_ENUM resp;
    _ssp_setup_command_structure(&setup, &sspC);
    sspC.CommandDataLength = 1;
    sspC.CommandData[0] = route;
    resp = _ssp_return_values(setup.port, &sspC);
